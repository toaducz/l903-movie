      ...
      {listMovie?.items?.length === 0 ? (
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-5 gap-3 sm:gap-5 p-3 w-full lg:px-24'>
          {listMovie?.items?.map(movie => (
            <div key={movie.slug}>
              <MovieItem movie={movie} color='bg-slate-900' source='kkphim' cdnDomain={listMovie?.APP_DOMAIN_CDN_IMAGE} />
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-center p-4 bg-slate-800'>
      <div className='flex flex-col pt-20 items-center'>
        <h2 className='text-2xl font-semibold text-[var(--c-pink)]'>KKPhim.com</h2>
        <div className='text-center italic text-white/70 text-[10px]'>(Nguồn KKPhim)</div>

        <form onSubmit={handleSearch} className='flex items-center space-x-2 py-5'>
          <input
            type='text'
            placeholder='Tìm theo tên phim (KKPhim)'
